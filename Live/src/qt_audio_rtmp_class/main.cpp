...
int main(int argc, char *argv[])
{
	...
	for (;;)
	{
		...
		//已经读一帧源数据
		//重采样源数据
		AVFrame *pcm = xMediaEncode->Resample(buf);
		...
		////推流
		xRtmp->SendFrame(pkt);
		//pkt->pts = av_rescale_q(pkt->pts, xMediaEncode->audio_codec_ctx->time_base, astream->time_base);
		//pkt->dts = av_rescale_q(pkt->dts, xMediaEncode->audio_codec_ctx->time_base, astream->time_base);
		//pkt->duration = av_rescale_q(pkt->duration, xMediaEncode->audio_codec_ctx->time_base, astream->time_base);
